...
void testcb(void* param, const char* item, uint8_t flags)
{
	xarc_item_info xi;
...
int main(int argc, char* argv[])
{
	...
	if (!xarc_ok(x))
	{
		fprintf(stderr, "(%d) %s\n    %s\n", xarc_library_error_id(x),
		 xarc_error_description(x), xarc_error_additional(x));
		xarc_close(x);
		return -1;
	}
	...
	do
	{
		xarc_result_t xr = xarc_item_extract(x, "C:\\JDevel\\xarc\\bin\\test_dir",
